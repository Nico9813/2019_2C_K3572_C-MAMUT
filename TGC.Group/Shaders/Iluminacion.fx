...
float4 ps_Item(PS_DIFFUSE_MAP input) : COLOR0
{
	...
	if (distancia0 < 450 || distancia1 < 450 || distancia2 < 450)
	{
		...
		finalColor = saturate((15 / distanciaMin) * texelColor + finalColor);
		if (finalColor.r == 1 && finalColor.g == 1 && finalColor.b == 1)
			return texelColor * coef + (texelColor * 0.78 + float4(0.90, 0.9, 0, 1) * 0.22) * (1 - coef);
		else
			return finalColor * coef + (finalColor * 0.78 + float4(0.90, 0.9, 0, 1) * 0.22) * (1 - coef);
	}
	else
		return finalColor * coef + (finalColor * 0.78 + float4(0.90, 0.9, 0, 1) * 0.22) * (1 - coef);
	//return texelColor;//Esto para probar sirve para ver todo sin oscuridad
...
technique Item
{
	pass Pass_0
	{
		...
		VertexShader = compile vs_3_0 vs_DiffuseMap();
		PixelShader = compile ps_3_0 ps_Item();
	}
}

VS_OUTPUT_DIFFUSE_MAP vs_DiffuseMap_AB(VS_INPUT_DIFFUSE_MAP input)
{
	VS_OUTPUT_DIFFUSE_MAP output;


	input.Position.x += (input.Position.y * cos(time)) * 0.05;
	input.Position.z += (input.Position.y * sin(time)) * 0.05;
	//Proyectar posicion
	output.Position = mul(input.Position, matWorldViewProj);
